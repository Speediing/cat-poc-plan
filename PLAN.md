...
> **Live frame (Aug 2026 leave-behind):** Aug 11 Dallas onsite. One work week (Mon–Fri): write the checks, agree the design, build in real repos, hand over, show the checks and decide. Plain voice. HOW WE RUN IT + WHAT WE PROVE high on the page. 30-day token trial is commercial only (The deal). See `CONTENT.md` / `src/lib/content.ts`. Sections below are earlier IA notes and may lag the live site.
